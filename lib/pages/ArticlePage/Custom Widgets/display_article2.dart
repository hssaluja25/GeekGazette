...
class DisplayArticle extends StatefulWidget {
  ...
  DisplayArticle(this.snapshotData, this.index) {
    currentArticle = snapshotData[index];
  }
...
class _DisplayArticleState extends State<DisplayArticle> {
  ...
  handleBookmarking() async {
    ...
    if (!_isBookmarked) {
      ...
      // bookmarked articles.
      if (!bookmarkedArticles.contains('${widget.currentArticle.id}')) {
        bookmarkedArticles.add('${widget.currentArticle.id}');
      ...
      bookmarkedArticles.remove('${widget.currentArticle.id}');
      prefs.setStringList(key, bookmarkedArticles);
      if (mounted) {
  ...
  handleOpeningUrl() async {
    ...
    if (connectivityResult == ConnectivityResult.wifi ||
      ...
      final urlOfArticle = Uri.parse('${widget.currentArticle.url}');
  ...
  openComments() async {
    ...
    if (connectivityResult == ConnectivityResult.mobile ||
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (BuildContext context) =>
              CommentsPage(commentIds: widget.currentArticle.kids),
        ),
      );
    } else {
      SnackBar snackbar =
    ...
    return Padding(
      ...
      child: ListTile(
        title: Column(
          ...
          children: [
            Text(
              '${widget.currentArticle.by} · ${formatDate(milliseconds: widget.currentArticle.time ?? 0)}',
              style: const TextStyle(fontSize: 14),
            ),
            Text('${widget.currentArticle.title}'),
            Row(
